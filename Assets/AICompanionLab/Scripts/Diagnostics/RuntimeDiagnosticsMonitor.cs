...
namespace Nyxara.AICompanion.Diagnostics
{
    public class RuntimeDiagnosticsMonitor : MonoBehaviour
    {
        ...
        private ComponentStatus CheckSTTStatus()
        {
            ...
            status.isPresent = true;
            status.isOperational = whisperInput.enabled && whisperInput.WhisperManager != null;
            status.statusMessage = status.isOperational ? "Ready for voice input" : "Disabled or missing WhisperManager";
        ...
        private ComponentStatus CheckTTSStatus()
        {
            ...
            status.isPresent = true;
            status.isOperational = ttsService.IsConfigured;
            status.statusMessage = status.isOperational ? "Configured and ready" : "Missing paths or files";
        ...
        private List<PathValidation> ValidatePaths()
        {
            ...
            {
                validations.Add(ValidatePath("LLM Model", llmAgent.llm.model));
            }

            if (whisperInput?.WhisperManager != null)
            {
                validations.Add(ValidatePath("Whisper Model", whisperInput.WhisperManager.ModelPath, true));
            }

            if (ttsService != null)
            {
                validations.Add(ValidatePath("Piper Executable", ttsService.PiperExecutablePath));
                validations.Add(ValidatePath("Piper Voice", ttsService.VoiceModelPath));
            }
        ...
        private List<ConfigIssue> FindConfigIssues()
        {
            ...
            if (llmAgent?.llm != null && string.IsNullOrEmpty(llmAgent.llm.model))
            {
            ...
            if (ttsService != null && ttsService.AudioSource == null)
            {
                issues.Add(new ConfigIssue
                {
                    severity = IssueSeverity.Warning,
